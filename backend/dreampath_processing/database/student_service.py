...
class StudentDatabaseService:
    ...
    async def save_student_profile(self, student_profile: StudentProfile, user_id: int) -> int:
        """Save student profile to PostgreSQL and set as active. Returns the profile ID."""
        # Insert new profile
        result = await self.db.execute_one(
            """
            INSERT INTO student_profiles (user_id, name, major, college_interests, post_grad_goals, career_goals)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING id
            """,
    ...
    async def load_student_profile(self, user_id: int, student_profile_id: Optional[int] = None) -> Optional[StudentProfile]:
        """Load student profile. Defaults to active profile for user, or specific profile if provided."""
        if student_profile_id is None:
            # Load active profile for user
            row = await self.db.execute_one(
                """
                SELECT sp.name, sp.major, sp.college_interests, 
                       sp.post_grad_goals, sp.career_goals
                FROM student_profiles sp
                JOIN users u ON u.active_profile_id = sp.id
                WHERE u.id = $1 AND sp.is_active = true
                """,
                user_id
            )
        else:
    ...
    async def save_course_path(self, course_path: CoursePath, user_id: int, pending_approval: bool = False) -> int:
        """Save course path to PostgreSQL and link to active profile. Returns course path ID."""
        # Get the active profile ID for this user
        ...
        if not active_profile or not active_profile['active_profile_id']:
            raise ValueError(f"No active profile found for user {user_id}")
        
        profile_id = active_profile['active_profile_id']
    ...
    async def load_course_path(self, user_id: int, pending_approval: bool = False, course_path_id: Optional[int] = None) -> Optional[CoursePath]:
